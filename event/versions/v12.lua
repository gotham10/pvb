...
local function isPlayerWhitelisted(playerName, whitelistTable)
    if not whitelistTable then return false end
    for _, entry in ipairs(whitelistTable) do
        if playerName == entry then
            return true
        end

        local base, rangeStart, rangeEnd = string.match(entry, "^(.-)(\d+)-(\d+)$")
        if base then
            local playerBase, playerNumStr = string.match(playerName, "^(.-)(\d+)$")
            if playerBase and playerBase == base then
                local playerNum = tonumber(playerNumStr)
                local startNum = tonumber(rangeStart)
                local endNum = tonumber(rangeEnd)
                if playerNum and startNum and endNum and playerNum >= startNum and playerNum <= endNum then
                    return true
                end
            end
        end
...
if not _G.Config or not isPlayerWhitelisted(p.Name, _G.Config.wl) then return end
...
local function runAutoSell()
    cleanup("AutoSell")
    if not _G.Config.AutoSellLimited and not _G.Config.AutoSellSecret then return end
    
    local connections = {}
    _G.ScriptManager.Connections["AutoSell"] = connections

    local b_limited = playerGui.Main.AutoSell.Frame.Limited.TextButton
    local b_secret = playerGui.Main.AutoSell.Frame.Secret.TextButton
    ...
    updateAutoSellSecret = function()
        ...
        if _G.Config.AutoSellSecret then
            shouldBeOn = _G.AutoSellSecret_Active
        end
    ...
    local thread = coroutine.create(function()
        while (_G.Config.AutoSellLimited or _G.Config.AutoSellSecret) and task.wait(3) do
...
local function runMissionBrainrotMonitor()
    ...
    local success, err = pcall(function()
        missionBrainrots = ws:WaitForChild("ScriptedMap"):WaitForChild("MissionBrainrots")
    end)
    if not success or not missionBrainrots then return end
...
B    end
    ...
    local function onChildAdded(child)
        if child.Name == "Hacktini Adminin" then
                ...
                if playerPlot:FindFirstChild("Brainrots") then
                    for _, brainrot in ipairs(playerPlot.Brainrots:GetChildren()) do
                        if brainrot:FindFirstChild("Hitbox") and brainrot.Hitbox:FindFirstChild("ProximityPrompt") then
                            local prompt = brainrot.Hitbox.ProxBbimityPrompt
