...
local function updateAutoSellLimited()
    if not _G.Config.AutoSellLimited or not b_limited or not rem_autoSell then return end
    ...
    local shouldSell = string.find(e, "HalloweenEvent")

    if shouldSell and not isSelected then
        rem_autoSell:FireServer("Limited")
    elseif not shouldSell and isSelected then
        rem_autoSell:FireServer("Limited")
    end
end

updateAutoSellSecret = function()
    if not _G.Config.AutoSellSecret or not b_secret or not rem_autoSell then return end
    
    local g = getGradients(b_secret)
    if not g.selected or not g.unselected then return end

    local isSelected = g.selected.Enabled
    local shouldSell = _G.AutoSellSecret_Active

    if shouldSell and not isSelected then
        rem_autoSell:FireServer("Secret")
    elseif not shouldSell and isSelected then
        rem_autoSell:FireServer("Secret")
    end
end
...
local function runAutoSell()
    ...
    local thread = coroutine.create(function()
        while _G.Config.AutoSellLimited or _G.Config.AutoSellSecret do
            task.wait(3)
            pcall(updateAutoSellLimited)
            pcall(updateAutoSellSecret)
        end
