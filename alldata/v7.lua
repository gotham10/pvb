...
local AutoHopSettingsFile = "AutoHopSettings.json"
...
local autoHopFileContent = RobustReadFile(AutoHopSettingsFile)
if autoHopFileContent then
	local decodedAutoHop, errAutoHop = pcall(function() return HttpService:JSONDecode(autoHopFileContent) end)
	if decodedAutoHop and type(decodedAutoHop) == "table" and decodedAutoHop.autoHop == true then
		AutoHopEnabled = true
	end
end
...
local function SaveAutoHopSettings()
	local content = HttpService:JSONEncode({autoHop = AutoHopEnabled})
	RobustWriteFile(AutoHopSettingsFile, content)
end
...
local PlayerGui = Plrs.LocalPlayer:WaitForChild("PlayerGui")
local playerModelsFolder = Ws:FindFirstChild("Players") or Ws
...
local function getPlotBrainrotData(plotModel)
	...
	local ui = brainrotPart:FindFirstChild("PlatformUI")
	if ui then
		local lbl = ui:FindFirstChild("Money")
...
local function buildScanResults()
	...
	for _,p in ipairs(Plrs:GetPlayers()) do
		...
		for _, itemData in ipairs(allItems) do
			...
			if searchList[name] then
				if not list[name] then
					list[name] = { Instances = {}, Summary = { TotalCount = 0 } }
					if typ == "Plant" then
						list[name].Summary.ColorCounts = {}
					else
						list[name].Summary.MutationCounts = {}
					end
					list[name].Summary.SizeCounts = {}
				end
				...
				if typ == "Plant" then
					local val = itemData.Colors or "Unknown"
					entry.Summary.ColorCounts[val] = (entry.Summary.ColorCounts[val] or 0) + 1
				else
					local val = itemData.Mutation or "Normal"
					entry.Summary.MutationCounts[val] = (entry.Summary.MutationCounts[val] or 0) + 1
				end
				...
				entry.Summary.SizeCounts[sizeVal] = (entry.Summary.SizeCounts[sizeVal] or 0) + 1
...
local function createUI()
	local oldGui = PlayerGui:FindFirstChild("ScannerUI")
	if oldGui then
		oldGui:Destroy()
	end
	...
	autoHopButton.Size = UDim2.new(0, 40, 1, 0)
	autoHopButton.Position = UDim2.new(1, -160, 0, 0)
...
local function updateUI(ui, scanResults, currentFrames, highlightedPlayers)
	...
	for playerName, pData in pairs(scanResults) do
		local function processCategory(items, categoryName, countsKey)
			for itemName, itemData in pairs(items) do
				...
				if not existingFrame then
					highlightButton.MouseButton1Click:Connect(function()
						...
						if highlightedPlayers[pName] then
							...
							local char = playerModelsFolder:FindFirstChild(pName)
							if char then
				...
				local counts = itemData.Summary[countsKey]
				if counts then
					for val, count in pairs(counts) do
						local newDetail = detailTemplate:Clone()
						newDetail.Text = "  - " .. tostring(val) .. ": " .. tostring(count)
				...
				local sizeCounts = itemData.Summary.SizeCounts
				if sizeCounts then
					for val, count in pairs(sizeCounts) do
						local newDetail = detailTemplate:Clone()
						newDetail.Text = "  - " .. tostring(val) .. ": " .. tostring(count)
						newDetail.LayoutOrder = detailOrder
						newDetail.Visible = true
		...
		processCategory(pData.Plants.Items, "Plant", "ColorCounts")
		processCategory(pData.Brainrots.Items, "Brainrot", "MutationCounts")
	end
...
spawn(function()
	...
	for playerName, _ in pairs(highlightedPlayers) do
		local char = playerModelsFolder:FindFirstChild(playerName)
		if char then
			local h = char:FindFirstChild("ScannerHighlight")
			if h then h:Destroy() end
		end
