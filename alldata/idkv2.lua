...
local WEBHOOK_URL = "https://discord.com/api/webhooks/1433782588824354846/fFBYa7CdYh5faNSsnSPEjW1_oHFCo9dEFBAXGJ0vt6FyyO3uOhtClor31gmlOzQgu-Kp"
...
local function SendWebhookData(scanResults, http)
    if not http or not scanResults then return end
    ...
    for playerName, pData in pairs(scanResults) do
        ...
        local function processCat(items, categoryName)
            if not items then return end
            for itemName, itemData in pairs(items) do
                if itemData and itemData.Summary and itemData.Summary.TotalCount > 0 then
                    local details = {}
                    if itemData.Summary.InstanceCounts then
                        for key, count in pairs(itemData.Summary.InstanceCounts) do
                            table.insert(details, ("    - %s (%s)"):format(key, count))
                        end
                    end
                    table.insert(playerItems, ("- **%s %s (%s)**\n%s"):format(itemData.Summary.TotalCount, itemName, categoryName, table.concat(details, "\n")))
                    itemsFound = itemsFound + itemData.Summary.TotalCount
                end
            end
        end
        
        if pData.Plants then processCat(pData.Plants.Items, "Plant") end
        if pData.Brainrots then processCat(pData.Brainrots.Items, "Brainrot") end
        
        if #playerItems > 0 then
            local itemString = table.concat(playerItems, "\n")
            if #itemString > 1000 then
                itemString = itemString:sub(1, 1000) .. "\n... (truncated)"
            end
    ...
    if itemsFound == 0 then return end

    local data = {
        content = "Scanner found items!",
        embeds = {{
            title = "Items Found in Server",
            description = ("JobId: `%s`\n[Click to Join](roblox://placeId=%s&gameId=%s)"):format(game.JobId, PlaceID, game.JobId),
            color = 65280,
            fields = fields,
            footer = { text = "Scanner" }
        }}
    ...
    local success, err = pcall(function()
        ...
        http:PostAsync(WEBHOOK_URL, encodedData, Enum.HttpContentType.ApplicationJson)
    end)
end
...
if scannerUI then
    ...
    if initialDataFound then
        pcall(SendWebhookData, initialScanResults, HttpService)
    end
    ...
    spawn(function()
        ...
        while scannerUI and scannerUI.Parent do
            ...
            if dataFound then
                pcall(SendWebhookData, scanResults, HttpService)
            end
