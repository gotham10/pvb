...
if autoHopFileContent then
	...
	if decodedAutoHop and type(decodedAutoHop) == "table" and decodedAutoHop.autoHop ~= nil then
		AutoHopEnabled = decodedAutoHop.autoHop
	end
end
...
local ScanPlants = {
	["Troll Mango"] = { kg = 0, mutation = nil },
	["Commando Apple"] = { kg = 0, mutation = nil },
	["King Limone"] = { kg = 3, mutation = nil }
}

local ScanBrainrots = {
	["Pacchetto Di Carte"] = { kg = 0, mutation = nil }
}
...
local function buildScanResults()
	...
	for _,p in ipairs(Plrs:GetPlayers()) do
		...
		for _, itemData in ipairs(allItems) do
			...
			local isOverGeneralThreshold = sizeInKg > generalThreshold
			...
			if foundSpecific or isOverGeneralThreshold or isGlobalMutationWhitelisted then
