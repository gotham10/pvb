...
local SCRIPT_URL = "https://raw.githubusercontent.com/gotham10/pvb/refs/heads/main/alldata/test5.lua"
...
if queueteleport then
    game:GetService("Players").LocalPlayer.OnTeleport:Connect(function(State)
        if KeepScriptRunning and (not TeleportCheck) then
            ...
            local scriptToQueue = configString .. "\n" .. "loadstring(game:HttpGet('" .. SCRIPT_URL .. "'))()"
            ...
            if brainrotsFolder then
                for _, plotModel in ipairs(brainrotsFolder:GetChildren()) do
                    if plotModel:IsA("Model") then
                        local d = getPlotBrainBlock(plotModel)
                        if d then table.insert(items, d) end
                    end
                end
            end
