			...
			if rewardFrame:IsA("Frame") and rewardFrame.Name ~= "Template" then
				...
				if content then
					...
					if icon and amount and title then
						local rewardTitle = title.Text
						local rewardAmount = amount.Text

						if rewardTitle == "Money" then
							rewardAmount = "It’s different for every account due to multipliers and other factors, so I can’t display it."
						end
